...
# st.dataframe(df.drop(columns=["gaps_list", "improvements_list", "lat", "lon"]).astype(str))
...
# # ----------------------------
# st.subheader("🗺 Village Map (Priority View)")
...
# # ----------------------------
# st.subheader("⚖ Compare 2 Villages")
...
# ----------------------------
# Load Village Data with fix for gaps_list
# ----------------------------
@st.cache_data
def load_data():
    df = pd.read_csv("sample_villages.csv")
    ...
    # Convert 'gaps_list' from string to list safely
    df['gaps_list'] = df['gaps_list'].apply(parse_list_column)

    # Convert priority_score to int safely
    df['priority_score'] = pd.to_numeric(df['priority_score'], errors='coerce').fillna(0).astype(int)

    # Convert gaps column to string to avoid mixed types
    df['gaps'] = df['gaps'].astype(str)
...
ADJUSTED_THRESHOLDS = {
    "schools_per_1000": st.sidebar.slider("Schools (per 1000 pop)", 1, 3, BASE_THRESHOLDS["schools_per_1000"]),
    "toilets_per_household": st.sidebar.slider(
        "Toilets (% HH Saturation)", 1.0, 1.2, BASE_THRESHOLDS["toilets_per_household"], step=0.1
    ),
    "PHCs_min": st.sidebar.slider("PHCs (Minimum)", 1, 3, BASE_THRESHOLDS["PHCs_min"]),
    "water_points_per_50_hh": st.sidebar.slider("Water Points (per 50 HH)", 1, 3, BASE_THRESHOLDS["water_points_per_50_hh"]),
    "electricity_hours_min": st.sidebar.slider("Electricity (Min Hours)", 10, 24, BASE_THRESHOLDS["electricity_hours_min"])
...
# Convert list columns to strings for display to avoid serialization errors
df['gaps_list_str'] = df['gaps_list'].apply(lambda x: ', '.join(x) if isinstance(x, list) else '')
df['improvements_list_str'] = df['improvements_list'].apply(lambda x: ', '.join(x) if isinstance(x, list) else '')

df["gaps"] = df["gaps_list_str"].replace('', 'None')
df["improvements"] = df["improvements_list_str"]
...
# Prepare display dataframe: drop columns with lists or problematic types
display_df = df.drop(columns=["gaps_list", "improvements_list", "gaps_list_str", "improvements_list_str", "lat", "lon"])
...
for i, row in df.dropna(subset=['lat', 'lon']).iterrows():
    ...
    folium.Marker(
        ...
        popup=popup_html,
        icon=folium.Icon(color=row["priority_color"])
    ).add_to(m)
...
def create_pdf(df):
    ...
    story.append(Paragraph("AdarshPulse Village Gap Report", styles["Title"]))
    story.append(Spacer(1, 12))
    
    for _, row in df.iterrows():
        text = f"<b>{row['village_name']} (ID: {row['village_id']})</b><br/>"
        text += f"Population: {row['population']}<br/>"
        text += f"Gaps: {row['gaps']}<br/>"
        text += f"Improvements: {row['improvements']}<br/><br/>"
...
if st.button("Export Report as PDF"):
    ...
    st.download_button(label="Download PDF", data=pdf_bytes, file_name="adarshpulse_report.pdf", mime="application/pdf")